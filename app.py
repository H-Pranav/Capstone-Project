...
def signin():
    if request.method=='POST':
        ...
        query='select uname,upass,name from user1 where uname=%s AND upass=%s'
...
def signup():
    if request.method=='POST':
        ...
        query='select uname from user1 where uname=%s'
...
def camreg():
    if 'signedin' in session:
        ...
        if request.method=='POST':
            ...
            query1="insert into user1 values(%s,%s,%s,%s)"
...
def vfeed():
    if 'login' in session: 
        una=session['un']
        query=f"select pic1 from user1 where uname = '{una}'"
